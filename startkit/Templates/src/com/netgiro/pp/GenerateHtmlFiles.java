...
public class GenerateHtmlFiles {
	...
	private static final String CONTEXT_DIR = "/Templates/context/";
	private static final String TEMPLATE_DIR = "/Templates/template/";
	...
	private static void genAll() throws Exception {
		...
		System.err.println("templ dir: "+templDir.getAbsolutePath());
	...
	static File genHtmlFromTemplate( File templ, Locale locale ) throws Exception {
		...
		File f = new File(GenerateHtmlFiles.class.getResource("/Templates/template").getFile());
	...
	private static Map<String,String> getStaticData( Locale locale ) {
		ResourceBundle.Control ctrl = new ExternalResourceBundleControl(
				GenerateHtmlFiles.class.getResource(ResourceBundle2Txt.RESOURCE_FOLDER).getFile());
	...
	private static Map<String, String> getDummyReqData() {
		...
		res.put("billingSocialSecurityNumber", "123-456-7890");
		res.put("bankSwiftBIC", "WERTSEDF");
		res.put("bankAccountIban", "DE00000123456789");
		res.put("mandateDate", "20140220");
		res.put("shippingAddressLine1", "Textilgatan 31 billing");
        res.put("shippingAddressLine2", "Second address line billing");
        res.put("shippingStreetName", "StreetName");
        res.put("shippingHouseNumber", "31");
        res.put("shippingHouseExtension", "31");
        res.put("shippingCity", "Stockholm billing");
        res.put("shippingZipCode", "144 80");
        res.put("shippingCountryCode", "SE");
		res.put("shippingEmailAddress", "niklas@dr.se");
		res.put("shippingFirstName", "Niklas");
		res.put("shippingLastName", "Pragsten");
		res.put("shippingPhone", "123456789");
		res.put("shippingMobilePhone", "123456789");
		res.put("consumerCountry", "SE");
		res.put("gender", "M");
		res.put("birthDate", "19000101");
		res.put("errorCode", "0");
		return res;
	}
	...
	private static Map<String,String> getDummyConfigData(Locale locale) {
		...
		res.put("get_payment_plan_servlet", "DUMMY");
		res.put("get_address_servlet", "DUMMY");
		return res;
	}
}

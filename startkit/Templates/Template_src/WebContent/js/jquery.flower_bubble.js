...
var flower_bubble = function ( options ) {
	...
	this . defaults = {
		...
		//base_dir: ${config.content_url}img,
		background: { css: 'white', opacity: 0.5 }//,
		//bubble: null,//{ image: 'bubble.png', width: 'auto', height: 'auto' },
		//flower: null//{ image: 'flower.gif', width: 'auto', height: 'auto' }
	} ;
	// end: define defaults
	...
	var t = new Date () ;
	// **
	// also added random number to make it work when several overlays being created at almost same time
	// important! do not remove; otherwise will not work properly in IE and Chrome!
	// **
	this . defaults . container_id += '_' + t . getTime () + Math.floor(Math.random()*100);
	...
	//var bubble_img = $( '<img src="' + this . bubble . image + '">' ) ;		//  + this . base_dir + '/'
	//var flower_img = $( '<img src="' + this . flower . image + '">' ) ;		//  + this . base_dir + '/'

...
public class DeviceRestApiExample {
	...
	public static void main(String[] args) {
		...
		PaymentHandler paymentHandler = new PaymentHandler(
				new JKSKeyHandlerV6("src/test/resources/netgiro-2048.jks", "123456", "netgiro", "netgiro"));

		PaymentRequest request = buildRequest();
		
		String encryptedPayload = paymentHandler.encryptRequest(request);
		...
		PaymentPageResponse response = paymentHandler.unpackResponse(respString);

		System.out.println("Response: mid=" + response.getMid() + ", orderid=" + response.getOrderId() + ", status="
				+ response.getStatus());
